    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if self.memory_mode == True:
            ...
            # 预分配目标tensor，减少内存碎片
            memory_tensor = torch.empty(batch_size, 3, memorys_length, views, N_mem, dim, 
                                        device=device, dtype=deepstack_memory_embeds[0].dtype)
            visual_tensor = torch.empty(batch_size, 3, views, N_vis, dim,
            ...
            for level, (mem_feat, vis_feat) in enumerate(zip(deepstack_memory_embeds, deepstack_visual_embeds)):
                ...
                memory_tensor[:, level] = mem_feat.view(batch_size, memorys_length, views, N_mem, dim)
                # vis: [B*2*N_vis, D] -> [B, 2, N_vis, D]
                visual_tensor[:, level] = vis_feat.view(batch_size, views, N_vis, dim)
